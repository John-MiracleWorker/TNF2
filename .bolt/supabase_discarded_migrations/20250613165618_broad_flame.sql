/*
  ...
  1. New Tables
    - `bible_reading_plans` - Store reading plan metadata
      - `id` (uuid, primary key)
      - `title` (text)
      - `description` (text)
    ...
    - `plan_daily_readings` - Store daily content for each plan
      - `id` (uuid, primary key)
    ...
    - `user_reading_progress` - Track user progress through plans
      ...
      - `created_at` (timestamptz)
      - Unique constraint on (user_id, plan_id)
    ...
    - `reading_reflections` - Store user reflections on daily readings
      ...
      - `ai_feedback` (text)
      - `created_at` (timestamptz)
      - Unique constraint on (user_id, plan_id, day_number)
