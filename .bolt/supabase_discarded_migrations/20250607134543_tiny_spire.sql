/*
  ...
  1. Changes
    - Create a proper `handle_new_user` function that creates all required records when a new user signs up
    - Create profiles record automatically
    - Create user_preferences record automatically
    - Create user_scripture_stats record automatically
    - Fix potential issues with the existing trigger function

  2. Security
    - Functions use SECURITY DEFINER to ensure they run with sufficient privileges
*/
...
-- Fix potential issues with existing triggers
DO $$
BEGIN
