/*
  ...
  1. New Tables
    - `bible_reading_plans` - Main reading plans
    - `plan_daily_readings` - Individual day content
    - `user_reading_progress` - Track user progress
    - `reading_reflections` - Store user reflections
  ...
  3. Sample Data
    - Add sample reading plans only if they don't already exist
*/

-- Create bible_reading_plans table if it doesn't exist
...
-- Create plan_daily_readings table if it doesn't exist
...
-- Create user_reading_progress table if it doesn't exist
...
-- Create reading_reflections table if it doesn't exist
...
-- Create policies with checks to prevent duplicate policy errors
DO $$
BEGIN
  -- Bible reading plans policy
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'bible_reading_plans' 
    AND policyname = 'Anyone can view active reading plans'
  ) THEN
  ...
  -- Plan daily readings policy
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'plan_daily_readings' 
    AND policyname = 'Anyone can view plan daily readings'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_reading_progress' 
    AND policyname = 'Users can insert their own reading progress'
  ) THEN
  ...
  -- Reading reflections policies
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'reading_reflections' 
    AND policyname = 'Users can view their own reading reflections'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'reading_reflections' 
    AND policyname = 'Users can insert their own reading reflections'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'reading_reflections' 
    AND policyname = 'Users can update their own reading reflections'
  ) THEN
...
DO $$
DECLARE
  prayer_journey_id UUID;
  spiritual_renewal_id UUID;
  plan_exists BOOLEAN;
BEGIN
  -- Check if any plans already exist
  SELECT EXISTS (
    SELECT 1 FROM bible_reading_plans 
    WHERE title = '7-Day Prayer Journey' OR title = '10-Day Spiritual Renewal'
  ) INTO plan_exists;
  
  -- If plans don't exist, insert sample data
  IF NOT plan_exists THEN
    -- Insert sample reading plans
    INSERT INTO bible_reading_plans (title, description, duration_days, theme) 
    VALUES
    ...
    -- Get IDs for plans we need to populate with daily readings
    SELECT id INTO prayer_journey_id FROM bible_reading_plans WHERE title = '7-Day Prayer Journey';
    SELECT id INTO spiritual_renewal_id FROM bible_reading_plans WHERE title = '10-Day Spiritual Renewal';
    
    -- Insert sample daily readings for the 7-Day Prayer Journey
    INSERT INTO plan_daily_readings (plan_id, day_number, title, description, scripture_reference, reflection_questions, prayer_prompt)
    VALUES
    (
      prayer_journey_id,
      1,
    ...
    -- Insert sample daily readings for the 10-Day Spiritual Renewal plan
    INSERT INTO plan_daily_readings (plan_id, day_number, title, description, scripture_reference, reflection_questions, prayer_prompt)
    VALUES
    (
      spiritual_renewal_id,
      1,
      'New Life in Christ',
    ...
    (
      ...
      'Moving forward requires releasing what is behind.',
      'Philippians 3:12-14',
