/*
  ...
  1. Changes
    - Drop all problematic policies causing infinite recursion 
    - Create new non-recursive policies for communities and members
    - Fix policies for viewing private vs public communities
    - Add proper member management and admin policies
    - Ensure no policy name conflicts by dropping existing policies first
  
  2. Security
    - Maintain the same security model but without recursion
    - Users can view public communities
    - Members can see private communities they belong to
    - Admins can manage community members
*/

-- Drop ALL potentially existing policies on community_members
...
DROP POLICY IF EXISTS "Members can view other members in same community" ON public.community_members;
DROP POLICY IF EXISTS "Users can view own memberships" ON public.community_members;
DROP POLICY IF EXISTS "Users can join new communities" ON public.community_members;
DROP POLICY IF EXISTS "Community admins can manage members" ON public.community_members;
DROP POLICY IF EXISTS "Global admins can view all members" ON public.community_members;
DROP POLICY IF EXISTS "Global admins can manage all members" ON public.community_members;

-- Drop ALL potentially existing policies on faith_communities
...
DROP POLICY IF EXISTS "Public communities are viewable by everyone" ON public.faith_communities;
DROP POLICY IF EXISTS "Private communities viewable by members" ON public.faith_communities;
DROP POLICY IF EXISTS "Authenticated users can create communities" ON public.faith_communities;
DROP POLICY IF EXISTS "Community admins can update their communities" ON public.faith_communities;
DROP POLICY IF EXISTS "Community admins can delete their communities" ON public.faith_communities;
