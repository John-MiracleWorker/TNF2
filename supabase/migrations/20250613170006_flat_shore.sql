/*
  # Bible Reading Plans Schema with Safe Data Insertion
  
  This migration creates the same tables as before but uses conditional
  logic to prevent duplicate inserts in the sample data.
*/
...
-- Insert sample reading plans for testing (conditionally to avoid duplicates)
DO $$
DECLARE
  ...
  plan_exists BOOLEAN;
BEGIN
  -- Check if we already have a 7-Day Prayer Journey plan
  SELECT EXISTS (
    SELECT 1 FROM bible_reading_plans 
    WHERE title = '7-Day Prayer Journey'
  ) INTO plan_exists;
  ...
  IF NOT plan_exists THEN
    ...
    -- Get the IDs of the inserted plans
